...
uses
  ...
  utreedb, uhistory, ucommon, uscript, LCLTranslator;
...
type
  ...
  private
    ...
    procedure DoStripTailSpace(InSelection: boolean = True);
    ...
    function  CopyNode(AToTree: TTreeView; AFromNode, AToNode: TTreeNode; AMode: TAttachMode): TTreeNode;
    procedure UpDownNode(Up: boolean);
    procedure MoveLeftNode;
    procedure MoveRightNode;
    procedure RenameNode;
    procedure SubmitRename(HideNamingBox: Boolean = True);
  ...
  public
    ...
    function  GetTotalHistorySize: integer; inline;
    ...
    procedure RenameNodes(ASearchText, ANameText: string; SearchInNote: boolean; Depth: Integer);
    procedure ScriptReplace(const ScriptText: string; InSelection: boolean); overload;
    procedure ScriptReplace(const ScriptText: string; Depth: integer); overload;

    procedure Search(ANode: TTreeNode; AText: string; IncludeName, IncludeNote: boolean; Depth: integer; IgnoreCase: boolean);
    procedure Replace(ANode: TTreeNode; AFrom, ATo: string; IncludeName, IncludeNote: boolean; Depth: integer; IgnoreCase: boolean);
    procedure RegSearch(ANode: TTreeNode; AText: string; IncludeName, IncludeNote: boolean; Depth: integer);
    procedure RegReplace(ANode: TTreeNode; AFrom, ATo: string; IncludeName, IncludeNote: boolean; Depth: integer);
...
resourcestring
  ...
  Res_DBIsOpened          = 'The database "%s" is already open in another program, still open it? If the same database file is opened in multiple programs, it may cause the data can''t be saved.';
...
begin
  ...
  if Application.HasOption('c', 'config') then
    ...
    FConfigDir := ConcatPaths([AppDir, DefConfigDir]);
  ...
  FEditHistory.CreateHistory(False);
...
begin
  if lstbInfo.Items.Count = 0 then Exit;
  if lstbInfo.ItemIndex <= 0 then
    lstbInfo.ItemIndex := lstbInfo.Items.Count - 1
  else
    lstbInfo.ItemIndex := lstbInfo.ItemIndex - 1;
  lstbInfoDblClick(lstbInfo);
  memoNote.SetFocus;
end;
...
begin
  if lstbInfo.Items.Count = 0 then Exit;
  if lstbInfo.ItemIndex >= lstbInfo.Items.Count - 1 then
...
begin
  SubmitRename(False);
  ANode := AddNode(trevTree, '', '', trevTree.Selected, naInsert);
  ANode.Selected := True;
...
begin
  SubmitRename(False);
  ANode := AddNode(trevTree, '', '', trevTree.Selected, naInsertBehind);
  ANode.Selected := True;
  SetNodeLoaded(ANode, True);
  RenameNode;
end;
...
begin
  SubmitRename(False);
  ANode := AddNode(trevTree, '', '', trevTree.Selected, naAddChildFirst);
  ANode.Selected := True;
  SetNodeLoaded(ANode, True);
  RenameNode;
end;
...
begin
  SubmitRename(False);
  ANode := AddNode(trevTree, '', '', trevTree.Selected, naAddChild);
  ANode.Selected := True;
  SetNodeLoaded(ANode, True);
  RenameNode;
end;
...
begin
  // 必须使用 FEditHistory.Paste，因为要处理历史记录
  if editRename.Visible then
    FEditHistory.Paste
  else
    FMemoHistory.Paste;
...
begin
  if editRename.Visible and FEditHistory.CanUndo then
    FEditHistory.Undo
  else if FMemoHistory.CanUndo then
    FMemoHistory.Undo;
end;
...
begin
  if editRename.Visible and FEditHistory.CanRedo then
    FEditHistory.Redo
  else if FMemoHistory.CanRedo then
    FMemoHistory.Redo;
end;
...
begin
  // 如果 editNanme 在激活时被隐藏，也有可能是活动控件，所以要做双重判断
  // 这里判断 ActiveControl 会促使 Action Update 及时更新
  if editRename.Visible and (ActiveControl = editRename) then begin
    actnCut       .Enabled := formMain.Active and (editRename.SelLength <> 0);
    actnCopy      .Enabled := actnCut.Enabled;
    actnPaste     .Enabled := formMain.Active and Clipboard.HasFormat(CF_TEXT);
    actnSelectAll .Enabled := formMain.Active and (editRename.Text <> '');
    actnDelete    .Enabled := actnCut.Enabled;
  end else if ActiveControl = memoNote then begin
...
begin
  ...
  actnMoveLeft      .Enabled := actnDeleteNode.Enabled;
  actnMoveRight     .Enabled := actnDeleteNode.Enabled;
...
begin
  ...
  if FTreeDB.Active then begin
    ...
    Config.AutoSaveRemaining := Config.AutoSaveInterval;
    if FTreeDB.DBConfig.AutoBackupRemaining = -1 then
      Config.AutoBackupRemaining := Config.AutoBackupInterval
    else
      Config.AutoBackupRemaining := FTreeDB.DBConfig.AutoBackupRemaining;
    Config.ChangedAfterBackup := FTreeDB.DBConfig.ChangedAfterBackup;
  end;
...
function TformMain.GetTotalHistorySize: integer; inline;
begin
...
var
  AText: string;
begin
  if Config.AutoStripTailSpace then begin
    // 此操作会处理历史记录
    DoStripTailSpace(False);
  end;
  if FNoteChanged and (FLastNode <> nil) then begin
    AText := ToLF(memoNote.Text);
    FTreeDB.SetNote(GetNodeID(FLastNode), AText); // Clear #13
  end;
...
begin
  ...
  if (ANode = nil) then begin
    ...
    if GetNodeHistory(ANode) = nil then
      SetNodeHistory(ANode, THistory.Create);

    FMemoHistory.SwitchHistory(GetNodeHistory(ANode));
  end;

  FLastNode := ANode;

  FMemoHistory.Enabled := FMemoHistory.History <> nil;
...
procedure TformMain.DoStripTailSpace(InSelection: boolean = True);
var
...
begin
  ...
  if InSelection and (SelLength > 0) then
    APrevText := memoNote.SelText
  else
    APrevText := memoNote.Text;

  // 这行代码需要开启 {$codepage UTF8}
  AText := StripTailSpace(APrevText);

  if Length(APrevText) > Length(AText) then begin
    FFixSelTextBug := True;
    FMemoHistory.Enabled := False;
    if InSelection and (SelLength > 0) then begin
      memoNote.SelText := AText;
      // AddRecord 会自动设置 FMemoHistory.FPrevText := FEdit.Text;
      // 所以需要先更新 memoNote.Text
      FMemoHistory.AddRecord(SelStart, SelLength, APrevText, SelStart, UTF8LengthFast(AText), AText);
    end else begin
      memoNote.Text := AText;
      // AddRecordSimply 会自动设置 FMemoHistory.FPrevText := FEdit.Text;
      // 所以需要先更新 memoNote.Text
      FMemoHistory.AddRecordSimply(APrevText);
    end;
    FMemoHistory.Enabled := True;
    if FFixSelTextBug then memoNoteChange(memoNote);
  end;
...
begin
  ...
  if n >= 0 then begin
    // 获取文件序号，并将文件移到最前面
    Index := StrToIntDef(Config.RecentFiles.Names[n], RecentFilesLimit);
    ...
    for i := 0 to Config.RecentFiles.Count - 1 do begin
      ...
      NewIndex := StrToIntDef(Config.RecentFiles.Names[i], RecentFilesLimit);
  ...
  for i := Config.RecentFiles.Count - 1 downto 0 do begin
    Index := StrToIntDef(Config.RecentFiles.Names[i], RecentFilesLimit);
    // 删除超出范围的文件项
    if Index >= RecentFilesLimit then begin
...
begin
  ...
  if not FTreeDB.OpenDB(AFileName) then begin
    Application.MessageBox(PChar(Res_OpenDBFail), PChar(AppTitle), MB_OK + MB_ICONERROR);
    Exit;
  end;

  // 为了防止程序意外关闭导致 DataBaseIsOpened 没有正确复位，这里需要
  // 一个“强行打开”的选择，否则数据库将永远无法打开。
  if FTreeDB.DBConfig.DataBaseIsOpened and
    (Application.MessageBox(PChar(Format(Res_DBIsOpened, [AFileName])), PChar(AppTitle), MB_YESNO + MB_ICONQUESTION) <> IDYES) then
  begin
    CloseDB(False);
    Exit;
  end;
  ...
  if Config.SelectLastNode then
    SelectNodeByID(FTreeDB.DBConfig.LastNodeID)
...
begin
  ...
  Config.AutoSaveRemaining := Config.AutoSaveInterval;
  FTreeDB.DBConfig.AutoBackupRemaining := Config.AutoBackupRemaining;
...
begin
  ...
  if FileExists(AFileName + '.tmp') then DeleteFile(AFileName + '.tmp');
...
begin
  ...
  FTreeDB.DBConfig.LastNodeID := GetNodeID(trevTree.Selected, FreeID);
  FTreeDB.DBConfig.AutoBackupRemaining := Config.AutoBackupRemaining;
...
begin
  ...
  if CurNode = nil then
  begin
    LoadSubNodes(ATree, ANode, Depth);
    if ANode = nil then
...
begin
  ...
  // 之后要执行 LoadNote，所以这里要先 SubmitNote，避免数据丢失
  SubmitNote;
  ...
  // 如果最后选择的是回收站中的节点，则清空回收站后也应该清空编辑框
  LoadNote(FLastNode);
...
procedure TformMain.MoveLeftNode;
var
  ID: integer;
  NextNode: TTreeNode;
begin
  ID := GetNodeID(trevTree.Selected);
  if ID = RootID then Exit;

  if FTreeDB.MoveLeft(ID) then begin
    trevTree.BeginUpdate;
    NextNode := trevTree.Selected.GetNextSibling;
    while NextNode <> nil do begin
      NextNode.MoveTo(trevTree.Selected, ComCtrls.naAddChild);
      NextNode := trevTree.Selected.GetNextSibling;
    end;
    trevTree.Selected.MoveTo(trevTree.Selected.Parent, ComCtrls.naInsertBehind);
    trevTree.Selected.Expand(False);
    trevTree.EndUpdate;
    DataStateChanged(True);
  end;
end;

procedure TformMain.MoveRightNode;
var
  ID: integer;
  ChildNode: TTreeNode;
begin
  ...
  if FTreeDB.MoveRight(ID) then begin
    trevTree.BeginUpdate;
    trevTree.Selected.MoveTo(trevTree.Selected.GetPrevSibling, ComCtrls.naAddChild);
    ChildNode := trevTree.Selected.GetLastChild;
    while ChildNode <> nil do begin
      ChildNode.MoveTo(trevTree.Selected, ComCtrls.naInsertBehind);
      ChildNode := trevTree.Selected.GetLastChild;
    end;
    trevTree.EndUpdate;
    DataStateChanged(True);
...
procedure TformMain.SubmitRename(HideNamingBox: Boolean);
begin
  if editRename.Visible then begin
    if FLastNode.Text <> editRename.Text then begin
      FTreeDB.SetName(GetNodeID(FLastNode), editRename.Text);
      FLastNode.Text := editRename.Text;
      DataStateChanged(True);
    end;
    if HideNamingBox then begin
...
var
  ...
  Found: Boolean;
  StartUPos, EndUPos: integer;

  function DoAddNode: boolean;
  begin
    Result := False;
    AContent := UTF8StringCopy(ANote, StartUPos, EndUPos);
...
begin
  ...
  SplitterPattern := RegUnEscape(SplitterPattern);
  TitlePattern := RegUnEscape(TitlePattern);
  ...
  try
    ...
    try
      while Found do begin
        EndUPos := Expr.MatchPos[0];

        if DoAddNode then Inc(Index);

        StartUPos := Expr.MatchPos[0];
        if not IncludeSplitter then
          Inc(StartUPos, Expr.MatchLen[0]);
...
procedure TformMain.RenameNodes(ASearchText, ANameText: string; SearchInNote: boolean; Depth: Integer);
var
  ID: integer;
begin
  SubmitNote;

  ID := GetNodeID(trevTree.Selected, RootID);

  SearchResult.Clear;
  FTreeDB.RenameNodes(ID, ASearchText, ANameText, SearchInNote, Depth, SearchResult);
  ReLoadNodeName;
  LoadSearchResult(False);
  DataStateChanged(SearchResult.Count > 0);
end;

procedure TformMain.ScriptReplace(const ScriptText: string; InSelection: boolean);
var
  Script: TScript;
  SelStart, SelLength: SizeInt;
  AText, APrevText, SelText: string;
...
begin
  ...
  try
    ...
    if Script.TextChanged then begin
      ...
      SelStart := memoNote.SelStart;
      SelLength := memoNote.SelLength;
      SelText := memoNote.SelText;
      if InSelection and (memoNote.SelLength > 0) then begin
        memoNote.SelText := AText;
        // AddRecord 会自动设置 FMemoHistory.FPrevText := FEdit.Text;
        // 所以需要先更新 memoNote.Text
        FMemoHistory.AddRecord(SelStart, SelLength, SelText, SelStart, UTF8LengthFast(AText), AText);
      end else begin
        memoNote.Text := AText;
        // AddRecordSimply 会自动设置 FMemoHistory.FPrevText := FEdit.Text;
        // 所以需要先更新 memoNote.Text
...
procedure TformMain.ScriptReplace(const ScriptText: string; Depth: integer); overload;
var
  ID: integer;
  i: integer;
  APrevText: string;
begin
  ...
  SearchResult.Clear;
  FTreeDB.ScriptReplace(ID, ScriptText, Depth, SearchResult);
  LoadSearchResult(False);
  DataStateChanged(SearchResult.Count > 0);
  // 重新载入当前节点，并处理历史记录
  if ID <> RootID then
    for i := 0 to Pred(SearchResult.Count) do
      if SearchResult[i]^.ID = ID then begin
        LoadNote(trevTree.Selected);
        // AddRecordSimply 会自动设置 FMemoHistory.FPrevText := FEdit.Text;
        FMemoHistory.AddRecordSimply(APrevText);
        break;
      end;
...
begin
  ...
  actnPrevSearchResult.Enabled := SearchResult.Count > 0;
  actnNextSearchResult.Enabled := actnPrevSearchResult.Enabled;
end;
...
procedure TformMain.Search(ANode: TTreeNode; AText: string; IncludeName, IncludeNote: boolean; Depth: integer; IgnoreCase: boolean);
var
  ID: integer;
begin
  SubmitNote;
  ...
  SearchResult.Clear;
  FTreeDB.Search(ID, AText, IncludeName, IncludeNote, Depth, IgnoreCase, Config.SearchCountLimit, SearchResult);
  LoadSearchResult(False);
end;

procedure TformMain.Replace(ANode: TTreeNode; AFrom, ATo: string; IncludeName, IncludeNote: boolean; Depth: integer; IgnoreCase: boolean);
var
  ID: integer;
  APrevText: String;
begin
  ...
  SearchResult.Clear;
  FTreeDB.Replace(ID, AFrom, ATo, IncludeName, IncludeNote, Depth, IgnoreCase, Config.SearchCountLimit, SearchResult);
  ...
  APrevText := memoNote.Text;
  LoadNote(FLastNode);
  if (Depth = 1) and (ANode = FLastNode) then
    // AddRecordSimply 会自动设置 FMemoHistory.FPrevText := FEdit.Text;
    FMemoHistory.AddRecordSimply(APrevText);
end;

procedure TformMain.RegSearch(ANode: TTreeNode; AText: string; IncludeName, IncludeNote: boolean; Depth: integer);
var
  ID: integer;
begin
  SubmitNote;
  ...
  SearchResult.Clear;
  FTreeDB.RegSearch(ID, AText, IncludeName, IncludeNote, Depth, Config.SearchCountLimit, SearchResult);
  LoadSearchResult(False);
end;

procedure TformMain.RegReplace(ANode: TTreeNode; AFrom, ATo: string; IncludeName, IncludeNote: boolean; Depth: integer);
var
  ID: integer;
  APrevText: String;
begin
  ...
  SearchResult.Clear;
  FTreeDB.RegReplace(ID, AFrom, ATo, IncludeName, IncludeNote, Depth, Config.SearchCountLimit, SearchResult);
  LoadSearchResult(False);
...
begin
  ...
  NewID := FTreeDB.ImportFile(FromPath, IncludeExt, ToID, mode, Config.AutoStripTailSpace);
...
begin
  ...
  ToID := GetNodeID(trevTree.Selected, RootID);
  NewID := FTreeDB.ImportDir(FromPath, IncludeRoot, IncludeExt, ToID, mode, Config.AutoStripTailSpace);
...
begin
  ...
  FTreeDB.ExportToFile(ID, ToPath, Splitter, Depth, Config.AutoStripTailSpace);
end;
...
begin
  ID := GetNodeID(Node, RootID);
  FTreeDB.ExportToDir(ID, ToPath, Ext, Depth, Config.AutoStripTailSpace);
end;
